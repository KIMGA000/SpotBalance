...
* 기상청, 한국관광공사 DataLab 등 공공데이터를 자동 수집·적재하는 데이터 파이프라인 구축.
* 사용자 취향, 날씨, 혼잡도를 통합한 개인화 추천 엔진 구현.
* 카카오 API 호출 제한에 대응하기 위해 OSRM 및 하버사인 거리 기반의 우회 경로 계산 로직 적용.
* 로그인 없이 Guest Session ID를 발급하여 사용자 행동 데이터를 수집·분석.
* 연령별 선호도를 월별 누적 평균 알고리즘으로 관리하여 관광 트렌드 변화를 반영.
* 최신 추천 가중치 테이블과 이력 로그 테이블을 분리하여 데이터 무결성과 추적 가능성 확보.
